...
- **URL + list–detail (clarified)**: For any **in-scope list–detail shell** (schools directory, school-scoped trips, trip workspace, passengers, payments, etc.), the intended shape is: **collection base** shows **LIST** + **DETAIL** placeholder (“select an item”); **after** a stable segment under that scope (e.g. a non-index child path), **LIST** stays paired with **DETAIL** for **that** shell only. **Do not** stack an unrelated collection list (e.g. the **schools directory**) beside a scoped hub such as `/schools/$schoolId/...`. Where the product avoids a bare `/$entityId` index URL for content, **redirect** to an explicit subpath (e.g. `.../home`, `.../summary`). **Close** / **Fechar** (compact or wide) **deselects** for **that** list–detail shell (parent handles routing), not a generic “back” metaphor; the dismiss control is part of the **detail region’s content** (typically beside the primary heading for that view), not an extra shell strip above the detail body.
- **Create/edit in the detail pane (FR-004)**: For shells that already have a **LIST** pane, **create** and **edit** routes for that collection (new school, new trip under a school, new passenger, new/edit payment, trip summary edit, etc.) render in the **DETAIL** region beside that list—not as a separate full-width column that hides the list. The **left** pane stays a **list** (or list-like primary nav for that scope), not a second “detail” column. **Exempt**: scoped school hub at `/schools/$schoolId/home` (no paired directory list; single main column by design).
...
6. **Given** a detail view that offers dismiss (clear selection / leave detail context), **When** the user looks at the detail region, **Then** that control sits in predictable relation to the detail title or primary heading for that screen.
7. **Given** a deep link or bookmark to an item that is missing or no longer accessible, **When** the screen loads, **Then** the detail region shows a clear unavailable/not-found state in `pt-BR`, the list remains usable where the layout allows, and the user can select another item or return to list context without a dead end.
...
- **FR-015**: Where the product provides a control to dismiss the detail context (deselect or move back toward the list on compact widths), that control MUST be rendered **inside** the detail region in a predictable relationship to the detail title or primary heading, and MUST follow the same interaction semantics (**Fechar** / clear selection, not vague “back” copy) called out elsewhere in this specification.
...
- **Detail surface**: The region used for viewing, editing, or creating the focused record within the list–detail pattern, including primary headings and any dismiss control offered for that view, as part of the detail content the user sees—not a separate empty chrome row from the layout shell alone.
...
- **SC-007**: During the two weeks after release, support or internal feedback channels show no new confusion reports specifically about “lost context” or “cannot get back to the list” on in-scope flows (target: zero such reports where none existed before).
