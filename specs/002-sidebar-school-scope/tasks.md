...
- [X] T023 [US2] Implement `../sitio-dashboard/src/components/layout/school-scope-header.tsx` (or split files) showing favicon/`img` with fallback avatar/initials, **title** as primary line, **placeholder username** secondary line per **FR-005** (**pt-BR** copy from `../sitio-dashboard/src/messages/pt-BR.ts`)
...
- [X] T026 [US2] Verify **icon-only** affordances use **`aria-label`** (**pt-BR**) for school edit button (implemented fully in US4) — stub disabled if needed until **T036**
