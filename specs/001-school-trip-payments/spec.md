...
- Q: How should duplicate passengers on the same trip be detected? → A: **Blocking duplicate error** when **CPF** (optional field) matches another passenger’s **CPF** on that trip after normalization (both values non-empty); **CPF** MUST be validated when provided (see FR-031, FR-038). **Full name** match when **no** CPF duplicate applies: **warning** plus required **confirmation** before save; staff may cancel or confirm to proceed.
...
- **CPF** on passengers is **optional** (some participants may not have one). When provided, **CPF** is validated (**FR-038**) and enforces **uniqueness within the trip** after normalization (**FR-031**). Normalization rules for **CPF** comparison are defined at implementation time and applied consistently on create and update.
