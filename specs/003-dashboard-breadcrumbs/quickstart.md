...
3. **Manual checks** (see [spec.md](./spec.md) success criteria):
   ...
   - Open **`/schools/{validUuid}/trips`** — trail includes **Viagens** as sidebar-aligned segment.
   - Open a **nested** trip route — **Viagens / [trip] / …** pattern; last segment visible without horizontal scroll on first paint when overflow exists.
